...
class Player:
    ...
    def download(self):
        ...
        json.dump(self.playlist, "playlistdata.json")
...
player = Player(json.load("playlistdata.json"))
...
playlist_list = tk.Listbox(root, listvariable=tk.Variable(value=list(player.playlist.keys())), height=len(player.playlist), exportselection=False)
playlist_list.bind("<<ListboxSelect>>", player.play)
playlist_list.grid(row=4, column=0, padx=40, pady=40)
